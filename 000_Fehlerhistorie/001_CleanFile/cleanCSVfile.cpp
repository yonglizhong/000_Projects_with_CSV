...
int main() {
    ...
    std::ifstream inputFile("Fehlerhistorie0005.csv"); // Open the input file named "test.txt" for reading
    std::ofstream outputFile("new_clean_test.csv"); // Create or overwrite the output file named "new_test.txt" for writing
    ...
    if (inputFile.is_open() && outputFile.is_open()) { // Check if both input and output files were successfully opened
        ...
        std::string searchWord = "License Key"; // Define the word to search for
        ...
        std::cout << "\nFile Cleaned with this many: " << removedCount << std::endl; // Display a success message
    } 
    else {
